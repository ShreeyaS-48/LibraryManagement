<?php
error_reporting(E_ALL);
ini_set('display_errors', 1);
session_start();
if((!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"]!=true) && $_SESSION["role"]=="member"){
    header("location: member_login.php");
    exit;
}
$showAlert = false;
$showError = false;
include 'partials/_dbconnect.php';
$bookSql = "SELECT book_id, book_title FROM book";
$bookResult = mysqli_query($conn, $bookSql);
$member_id = (int) $_SESSION['id'];
$profileSql = "SELECT * FROM `member` WHERE member_id = $member_id";
$profileResult = mysqli_query($conn, $profileSql);
$profileRow = mysqli_fetch_assoc($profileResult);
$active_status_id = $profileRow['active_status_id'];
$membershipSql = "SELECT * FROM `member_status` WHERE active_status_id = $active_status_id";
$membershipResult = mysqli_query($conn, $membershipSql);
$membershipRow = mysqli_fetch_assoc($membershipResult);
$account_status = $membershipRow['account_status'];
if($account_status === "Expired"){
    header("location: member_dashboard.php");
    exit;
}

    if($_SERVER["REQUEST_METHOD"] == "POST"){
        $book_id = $_POST['book_id']; 
        $member_id = $_SESSION['id'];
        $bookCheck = "SELECT copies_available FROM `book` WHERE book_id = '$book_id'";
        $result = mysqli_query($conn, $bookCheck);
        $row = mysqli_fetch_assoc($result);
        if ($row['copies_available'] > 0) {
            // Book is available
            $available_status = 'Available';
            $nearest_available_date = NULL;
            $showAlert = "Book Available!";
        }
        else {
            // Book is not available
            $available_status = 'Not Available';
        
            // Estimate next available date (from book_issue table)
            $dateQuery = "SELECT MIN(return_date) AS next_return FROM book_issue WHERE book_id = '$book_id'";
            $dateResult = mysqli_query($conn, $dateQuery);
            $dateRow = mysqli_fetch_assoc($dateResult);
            $nearest_available_date = $dateRow['next_return'] ?? NULL;

            $showError = "Book unavailable. Try again after date: " . $nearest_available_date;
        }
        $statusSql = "INSERT INTO book_request_status (available_status, nearest_available_date) 
              VALUES ('$available_status', " . ($nearest_available_date ? "'$nearest_available_date'" : "NULL") . ")";
        mysqli_query($conn, $statusSql);

        $available_status_id = mysqli_insert_id($conn);


        $request_date = date('Y-m-d');
        $insertRequest = "INSERT INTO book_request (book_id, member_id, request_date, available_status_id) 
                        VALUES ('$book_id', '$member_id', '$request_date', '$available_status_id')";
        mysqli_query($conn, $insertRequest);
    }

?>

<!doctype html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <!-- Bootstrap CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">

    <title>Request a Book</title>
  </head>
  <body>
    <?php require 'partials/_nav.php'?>
    <?php
        if($showAlert){
            echo '
            <svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
            <symbol id="check-circle-fill" fill="currentColor" viewBox="0 0 16 16">
                <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
            </symbol>
            </svg>
            <div class="alert alert-success alert-dismissible d-flex align-items-center fade show" role="alert">
            <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Success:"><use xlink:href="#check-circle-fill"/></svg>
            <div>'. htmlspecialchars($showAlert) .'</div>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div> ';
        }
        if($showError){
            echo '
            <svg xmlns="http://www.w3.org/2000/svg" style="display: none;">
            <symbol id="exclamation-triangle-fill" fill="currentColor" viewBox="0 0 16 16">
                <path d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
            </symbol>
            </svg>
            <div class="alert alert-danger alert-dismissible d-flex align-items-center fade show" role="alert">
            <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Success:"><use xlink:href="#exclamation-triangle-fill"/></svg>
            <div>'. htmlspecialchars($showError) .'</div>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div> ';
        }        
    ?>
    <div class="container my-4">
        <h1 class="text-center">Request a Book</h1>
        <form action="/php_project/issue_request.php" method="post">
        <div class="mb-3">
            <select class="form-select" name="book_id" id="book_id" required>
                <option value="" selected disabled>Choose a book</option>
                <?php while ($row = mysqli_fetch_assoc($bookResult)) { ?>
                    <option value="<?= $row['book_id'] ?>">
                        <?= htmlspecialchars($row['book_title']) ?>
                    </option>
                <?php } ?>
            </select>
        </div>
        
            <button type="submit" class="btn btn-primary">Request Book</button>
        </form>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
</body>
</html>