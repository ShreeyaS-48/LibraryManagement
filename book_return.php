...
if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($memberResult){
            ...
            $bookIssueSql = "SELECT * FROM `book_issue` WHERE member_id = $member_id AND book_id = $book_id AND return_status = '$return_status'";
            $bookIssueResult = mysqli_query($conn, $bookIssueSql);
            $numExistsRows = mysqli_num_rows($bookIssueResult);
            if($numExistsRows > 0){
                $bookIssueRow = mysqli_fetch_assoc($bookIssueResult);
                $issue_id = $bookIssueRow['issue_id'];
                $return_status = "Returned";
                $returned_on = date('Y-m-d');
                $bookReturnSql = "UPDATE `book_issue` SET returned_by_id = '$staff_id', returned_on = '$returned_on', return_status = '$return_status' WHERE issue_id = '$issue_id'";
                $bookReturnResult = mysqli_query($conn, $bookReturnSql);
                if($bookReturnResult){
                    ...
                    if($updateCopiesResult){
                        $return_date = $bookIssueRow['return_date'];
                        if($returned_on > $return_date){    
                            $showError = "Fine for late return.";
                        ...
                        $showError = "Error in updating copies";
                    }
                }
                else{
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container my-4">
        ...
        <form action="/php_project/book_return.php" method="post">
        <div class="mb-3">
